...
import { X, RotateCw, RotateCcw, Download, Save, FileOutput, FolderOpen } from 'lucide-react';
...
export const RotatePdf: React.FC<RotatePdfProps> = ({ onClose = () => {}, onAddFile, isDark = false, files = [], file }) => {
  ...
  const handleRotate = async (direction: 'cw' | 'ccw') => {
    ...
    try {
      ...
      const degChange = direction === 'cw' ? 90 : -90;

      pages.forEach(page => {
        const currentRot = page.getRotation().angle;
        page.setRotation(degrees(currentRot + degChange));
      });

      const pdfBytes = await pdfDoc.save();
      const compiledBlob = new Blob([pdfBytes], { type: 'application/pdf' });
      const secureName = `${selectedPdf.name.replace(/\.pdf$/i, '')}_rotated.pdf`;
      ...
      const fileId = 'pdf-rotate-item-' + Math.random().toString(36).substring(2, 11);
      ...
      NotificationManager.success(`Rotated and saved as "${secureName}".`);
    } catch (err) {
      console.error('Rotation failed', err);
      NotificationManager.error('Could not rotate PDF pages.');
    } finally {
    ...
    <div className={`flex flex-col h-full bg-transparent`}>
      <div className={`flex items-center justify-between px-6 py-4 border-b ${isDark ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-200'}`}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold text-lg font-sans">Rotate PDF</h2>
            <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Rotate all pages in document</p>
          </div>
        </div>
        ...
        <div className="w-full max-w-3xl mx-auto flex flex-col gap-6">
          ...
          <div className={`p-6 rounded-2xl flex flex-col items-center justify-center border-2 border-dashed ${isDark ? 'bg-[#1C1C1E] border-gray-800' : 'bg-white border-gray-200'}`}>
            {selectedPdf ? (
              <div className="w-full flex flex-col gap-4">
                <div className={`flex items-center justify-between p-4 rounded-xl border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                  <div className="flex items-center gap-3 overflow-hidden">
                    <FileOutput className="w-6 h-6 text-blue-500 flex-shrink-0" />
                    <div className="truncate">
                      <p className="font-medium text-sm truncate">{selectedPdf.name}</p>
                      <p className={`text-xs ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>{(selectedPdf.size / 1024 / 1024).toFixed(2)} MB</p>
                    </div>
                  </div>
                ...
                <div className="flex justify-center gap-4 mt-2">
                  <button onClick={() => handleRotate('ccw')} disabled={isProcessing} className="flex-1 flex gap-2 justify-center items-center py-3 bg-gray-100 hover:bg-gray-200 text-gray-800 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-gray-200 font-bold rounded-xl active:scale-95 transition-transform disabled:opacity-50">
                    <RotateCcw className="w-4 h-4" /> Rotate Left
                  </button>
                  <button onClick={() => handleRotate('cw')} disabled={isProcessing} className="flex-1 flex gap-2 justify-center items-center py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl active:scale-95 transition-transform disabled:opacity-50">
              ...
              <div className="py-6 flex flex-col items-center text-center">
                ...
                <h3 className="font-bold mb-2">Select a PDF to rotate</h3>
                <p className={`text-sm mb-6 ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Choose a document from your workspace.</p>
                <div className="flex gap-3">
                  <button onClick={() => setShowWorkspacePicker(true)} className="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-full font-semibold shadow-sm transition active:scale-95">
          ...
          {outputUrl && (
             <div className={`p-6 border rounded-2xl flex flex-col items-center justify-center text-center ${isDark ? 'bg-emerald-900/10 border-emerald-900/50' : 'bg-emerald-50 border-emerald-100'}`}>
               ...
               <h3 className="font-bold text-emerald-800 dark:text-emerald-400 mb-1">Rotation Applied</h3>
               <p className="text-sm text-emerald-600 dark:text-emerald-500 mb-5 text-center px-4">
                 Your rotated PDF "{outputName}" has been saved automatically to the Workspace.
               </p>
